...
TYPED_TEST(SdbusTestObject, CanRegisterAdditionalVTableDynamicallyAtAnyTime)
{
    ...
    vtable.emplace_back(sdbus::registerMethod("add").implementedAs([](const double& lhs, const double& rhs){ return lhs + rhs; }));
    vtable.emplace_back(sdbus::registerMethod("subtract").implementedAs([](const int& lhs, const int& rhs){ return lhs - rhs; }));
    auto vtableSlot = object.addVTable(interfaceName, std::move(vtable), sdbus::return_slot);
